...
function ivaldi_mail_collector_loadplugin(){
    ...
    register_widget_control('ivaldi_mail_collector_frontend', 'ivaldi_mail_collector_control', 200, 200 );
}
...
?>  
    ...
    <input class="widefat" type="text" id="ivaldi_mail_collector-WidgetIntro" name="ivaldi_mail_collector-WidgetIntro" value="<?php echo $options['intro'];?>" /></p>
...
<?php
		...
		echo '<p class="ivaldi_mail_collector-intro">'.$options['intro'].'</p>';
		echo '<form action="'.$_SERVER['REQUEST_URI'].'" method="post">';	
		echo '<label for="ivaldi_mail_collector_emailfield">'.__('Email').'<span>:</span></label> ';
		echo '<input type="text" id="ivaldi_mail_collector_emailfield" name="ivaldi_mail_collector_emailfield" />';
		echo '<input type="submit" class="btn" value="'.__('Add').'" />';
		echo '</form></div>';
	}
	echo $after_widget;
} 
...
load_plugin_textdomain('ivaldi_mail_collector_frontend',false, dirname( plugin_basename( __FILE__ ) ) . '/languages/');
